...
function concatenateStrings(array, subString) {
  let str = array.toString();
  let result = array.includes(subString);
  console.log(result);
...
function multiplyByIndex(arr) {
  ...
  for (const num of arr) {
    result[resultIndex] = num * resultIndex;
    resultIndex++;
  }
...
function replaceNumbers(arr) {
  ...
  for (const num of arr) {
    if (num > 10) {
      result[resultIndex] = "Greater than 10";
    } else {
      result[resultIndex] = "Less than or equal to 10";
    }
    resultIndex++;
  }
